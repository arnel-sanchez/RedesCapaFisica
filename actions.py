...
def send(signal_time: int, physical_layer: PhysicalLayer, instruction: Instruction, transmitting: list):
    ...
    for i in range(len(data)):
        if data[i] != 0 and data[i] != 1:
            ...
            raise Exception
    transmitting.append(physical_layer.send(signal_time, instruction.time, host, data))
    file = open("output/general.bin", 'a')
    file.write("time={}, instruction=send, host={}, data={}\n".format(instruction.time, host, instruction.details[1]))
    file.close()


def disconnect(physical_layer: PhysicalLayer, time: int, instruction: Instruction, transmitting: list):
